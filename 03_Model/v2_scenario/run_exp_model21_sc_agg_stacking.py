"""
모델실험21: 시나리오 집계 피처 + 5모델 스태킹 (v2.0 시작)
=========================================================
원자 단위 분석 결과:
  - 시나리오 간 분산 63.4%, 시나리오 내 36.6%
  - 기존 base 모델은 모든 구간에서 ~15분 상수 예측 (시나리오 레벨 구분 실패)
  - 시나리오 집계 피처(mean/std/max/min/diff) 추가 시 단일 LGBM MAE 8.62 (5모델 스태킹 8.79 초월)

핵심 변경:
  - FE v1 (lag/rolling) + 시나리오 집계 피처 90종 (18개 피처 × 5통계)
  - test에서도 시나리오 25행 피처를 전부 볼 수 있으므로 리크 아님
  - 5모델(LGBM+TW1.8+CB+ET+RF) 전체 재학습 (피처 변경 → 체크포인트 호환 불가)

예상:
  - 단독 LGBM: 8.62 (검증 완료) → 스태킹 시 8.5x 기대
  - base 모델이 시나리오 레벨을 구분하므로 메타 학습기 부담 경감

실행: python src/run_exp_model21_sc_agg_stacking.py
예상 시간: ~90분 (5모델 × 5fold + 메타)
출력: submissions/model21_sc_agg_stacking.csv
체크포인트: docs/model21_ckpt/
"""
...
CKPT_DIR = os.path.join(_BASE, '..', 'docs', 'model21_ckpt')
N_SPLITS = 5
RANDOM_STATE = 42

# ─────────────────────────────────────────────
# 시나리오 집계 피처 대상 (원자 분석에서 확인된 핵심 피처)
# ─────────────────────────────────────────────
SC_AGG_COLS = [
...
def main():
    t0 = time.time()
    print('=' * 60)
    print('모델실험21: 시나리오 집계 FE + 5모델 스태킹 (v2.0)')
    print('기준: RF5 스태킹 CV 8.7911 / Public 10.2213')
    print('검증 A 결과: 단일 LGBM 8.6237 (시나리오 집계 FE)')
    print('=' * 60)
    ...
    sub_path = os.path.join(SUB_DIR, 'model21_sc_agg_stacking.csv')
    ...
    print('\n' + '=' * 60)
    print(f'모델실험21 결과 ({elapsed:.1f}분 소요)')
    print('=' * 60)
    print(f'  LGBM  OOF MAE : {mae_lg:.4f}')
    ...
    print(f'  ')
    print(f'  기존 RF5 스태킹: 8.7911 (Public 10.2213, 배율 1.1627)')
    print(f'  변화             : {mae_meta - 8.7911:+.4f}')
