...
def main():
    ...
    # ── 순서 정렬 핵심 ──
    # GBDT OOF: build_features 후 저장 → scenario_id+ts_idx 정렬 순서
    # 시퀀스 OOF: sorted(scenario_ids) × 25 ts → 동일한 정렬 순서
    # ∴ 둘 다 scenario_id 정렬 순서이므로 재정렬 불필요!
    oof_cnn_flat  = np.expm1(oof_cnn).flatten()   # raw space, scenario-sorted
    oof_lstm_flat = np.expm1(oof_lstm).flatten()   # raw space, scenario-sorted

    # GBDT OOF 로드 (scenario_id 정렬 순서, build_features 출력 기준)
    ...
    # 순서 검증: 시퀀스 OOF MAE와 직접 계산 비교
    train_s = train.sort_values(['scenario_id', 'ID'])
    y_sorted = train_s['avg_delay_minutes_next_30m'].values
    cnn_check_mae  = np.abs(oof_cnn_flat - y_sorted).mean()
    lstm_check_mae = np.abs(oof_lstm_flat - y_sorted).mean()
    lgbm_check_mae = np.abs(oof_lgbm - y_sorted).mean()
    print(f'  순서 검증: CNN MAE={cnn_check_mae:.4f} (학습={mae_cnn:.4f})')
    print(f'  순서 검증: LSTM MAE={lstm_check_mae:.4f} (학습={mae_lstm:.4f})')
    print(f'  순서 검증: LGBM MAE={lgbm_check_mae:.4f} (기대: ~8.62)')
    if abs(cnn_check_mae - mae_cnn) > 0.5:
        print(f'  ⚠️ CNN MAE 불일치! 순서 문제 가능성')
    if abs(lgbm_check_mae - 8.62) > 1.0:
        print(f'  ⚠️ LGBM MAE 불일치! OOF 순서 확인 필요')

    corr_matrix = analyze_correlations(
        oof_cnn_flat, oof_lstm_flat,
    ...
    # flat OOF (scenario_id 정렬 순서, raw space) — 메타 학습기용
    # GBDT OOF와 동일 순서이므로 직접 column_stack 가능
    np.save(os.path.join(CKPT_DIR, 'cnn_oof_flat.npy'), oof_cnn_flat)
    np.save(os.path.join(CKPT_DIR, 'lstm_oof_flat.npy'), oof_lstm_flat)

    # test flat 변환 (scenario_id 정렬 순서 유지 — GBDT test와 동일)
    test_cnn_flat  = np.expm1(test_cnn).flatten()
    test_lstm_flat = np.expm1(test_lstm).flatten()

    np.save(os.path.join(CKPT_DIR, 'cnn_test_flat.npy'), test_cnn_flat)
    np.save(os.path.join(CKPT_DIR, 'lstm_test_flat.npy'), test_lstm_flat)
